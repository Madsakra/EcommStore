...
import com.example.product_store.kafka.service.RollBackPayment;
...
public class SagaOrchestra {
    ...
    public SagaOrchestra(KafkaTemplate<String, Object> kafkaTemplate, RollBackPayment rollBackPayment, InventoryRestockService inventoryRestockService) {
    ...
    public void handlerOrderEvent(OrderCreatedEvent event) {
        logger.info("Received Order at Saga Orchestra. Time of receipt:{}", LocalDateTime.now());
    ...
    public void handlePaymentCompletion(PaymentCompletedEvent paymentCompletedEvent){
        ...
        SagaEvents events = sagaEvents.computeIfAbsent(
    ...
    private void checkSagaCompletion(SagaEvents events){
        ...
        try {
            ...
            if (paymentEvent == null || inventoryEvent == null) {
                ...
                logger.info("Waiting for both payment and inventory events for order: {}", events.getOrderId());
                return;
            }
