...
public class OrderExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleOrderNotFoundException(OrderNotFoundException ex){
        return ErrorResponseTemplate.buildResponseError("Order not found with the given id",ex.getMessage(), HttpStatus.CONFLICT);
    }

    @ExceptionHandler
    public ResponseEntity<ErrorResponse> handleJsonProcessingError(JsonProcessingException ex){
        return ErrorResponseTemplate.buildResponseError("Internal Error Serializing data",ex.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
