...
public class DeleteUserFavouritesTest {
  ...
  @InjectMocks private DeleteUserFavouriteService deleteUserFavouriteService;
  ...
  void testExecute_shouldReturnVoid() {
    ...
    // ACT
    deleteUserFavouriteService.execute(mockedUserId, productId);

    // ASSERT
    assertFalse(
        mockAccount.getFavouriteProducts().contains(mockProduct),
        "Product should be removed from favourites");
  ...
  void testExecute_ProductNotFound_shouldThrowProductNotFoundException() {
    ...
    mockedAccount.setId(expectedJti);
    when(accountRepository.findById(expectedJti)).thenReturn(Optional.of(mockedAccount));
